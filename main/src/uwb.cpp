...
static esp_err_t get_time(uint64_t *tv, spi_device_handle_t dev_handle,
                          uint8_t reg = DWM_REG_SYSTEM_TIME_COUNTER) {
...
// TODO if still not working add subregister read and check LDERUNE when
// receiving
static void NodeA(spi_device_handle_t dev_handle) {
  ...
  do {
    ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_SYSTEM_EVENT_STATUS,
                                 (uint8_t *)&sys_status, sizeof(sys_status),
                                 dev_handle));
    log("polled ldedone bit: %X %X %X %X %X", sys_status[4], sys_status[3],
        sys_status[2], sys_status[1], sys_status[0]);
  ...
  //  TODO: this should technically be uint8_t[15] but it's easier to work with
  //  uint64_t
  uint64_t time_msg[3];
  time_msg[0] = TSP;
  time_msg[1] = TRR;
  ...
  ESP_ERROR_CHECK(uwb_delayed_transmit((uint8_t *)time_msg, 24, current_time,
                                       DELAY_CONST_MS, dev_handle));
...
static void NodeB(spi_device_handle_t dev_handle) {
  ...
  ESP_ERROR_CHECK(uwb_delayed_transmit((uint8_t *)PING_MSG, PING_MSG_LEN,
                                       current_time, DELAY_CONST_MS,
                                       dev_handle));
  ...
  // account for wrap, calculate responder round trip delay time TART and
  // responder responce time. TRRT
  ...
  double tof_us = (delta_time_us) / 2.0; // tof one-way in microseconds
  double distance_ft = tof_us * 983.57105643045;
  log("TRT(Responder Round Trip Time) is %llu, and TART(Initiator Round Trip "
      "Delay time) is %llu",
      TRRT, TART);
  log("Responder response time is %llu, Initiator Response time is %llu",
      (TSR - TRP), (TSF - TRR));
...
void uwb_init() {
  ...
  uint8_t rx_buf[4];
  log("SPI transaction: %d",
      uwb_read_reg(DWM_REG_DEV_ID, rx_buf, 4, dev_handle));
  ...
  uwb_write_subreg(DWM_REG_POWER_MANAGEMENT, 0x00, (uint8_t *)buf, sizeof(buf),
                   dev_handle);

  // Step 2
  buf[1] = 0x80;
  buf[0] = 0x00;
  uwb_write_subreg(DWM_REG_OTP_MEMORY, DWM_SUB_REG_OTP_CTRL, (uint8_t *)buf,
                   sizeof(buf), dev_handle);
  ...
  uint8_t sys_status[5];
  ESP_ERROR_CHECK(uwb_read_subreg(DWM_REG_SYSTEM_EVENT_STATUS, 0x00,
                                  (uint8_t *)&sys_status, sizeof(sys_status),
                                  dev_handle));
  log("polled sys status: %X %X %X %X %X", sys_status[4], sys_status[3],
      sys_status[2], sys_status[1], sys_status[0]);
  ...
  // ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_SYSTEM_EVENT_STATUS,
  // (uint8_t*)&sys_status, sizeof(sys_status), dev_handle)); log("polled sys
  // status: %X %X %X %X %X", sys_status[4], sys_status[3], sys_status[2],
  // sys_status[1], sys_status[0]);
...
esp_err_t uwb_read_reg(uint8_t reg, uint8_t *rx, size_t len,
                       spi_device_handle_t dev_handle) {
...
esp_err_t uwb_read_subreg(uint8_t reg, uint8_t subreg, uint8_t *rx, size_t len,
                          spi_device_handle_t dev_handle) {
  // Lower 6 bits store actual register
  // MSbit = 0 represents read
...
esp_err_t uwb_write_reg(uint8_t reg, uint8_t *tx, size_t len,
                        spi_device_handle_t dev_handle) {
  // Lower 6 bits store actual register
...
// TODO: needs to take 16-bit subreg
esp_err_t uwb_write_subreg(uint8_t reg, uint8_t subreg, uint8_t *tx, size_t len,
                           spi_device_handle_t dev_handle) {
  // Lower 6 bits store actual register
  // MSbit = 1 represents write
  // Second MSbit = 1 represent sub-index is present
...
esp_err_t uwb_transmit(uint8_t *tx, size_t len,
                       spi_device_handle_t dev_handle) {
  // Clear TXFRS bit (and other relevant flags)
  uint64_t sys_status_mask = DWM_SYS_STATUS_TXFRB | DWM_SYS_STATUS_TXPRS |
                             DWM_SYS_STATUS_TXPHS | DWM_SYS_STATUS_TXFRS;
  ESP_ERROR_CHECK(uwb_write_reg(DWM_REG_SYSTEM_EVENT_STATUS,
                                (uint8_t *)&sys_status_mask, 5, dev_handle));

  // Write payload data
  ESP_ERROR_CHECK(uwb_write_reg(DWM_REG_TRANSMIT_DATA_BUFFER, (uint8_t *)tx,
                                len, dev_handle));

  // Grab current transmit frame control register
  uint32_t transmit_frame_control;
  ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_TRANSMIT_FRAME_CONTROL,
                               (uint8_t *)&transmit_frame_control,
                               sizeof(transmit_frame_control), dev_handle));
  ...
  ESP_ERROR_CHECK(uwb_write_reg(DWM_REG_TRANSMIT_FRAME_CONTROL,
                                (uint8_t *)&transmit_frame_control,
                                sizeof(transmit_frame_control), dev_handle));

  // Write TXSTRT bit
  dwm_system_control_t sys_ctrl = DWM_SYS_CTRL_TXSTRT;
  ESP_ERROR_CHECK(uwb_write_reg(DWM_REG_SYSTEM_CONTROL, (uint8_t *)&sys_ctrl,
                                sizeof(sys_ctrl), dev_handle));
...
esp_err_t uwb_delayed_transmit(uint8_t *tx, size_t len,
                               uint64_t current_time_dtu, uint64_t delay_ms,
                               spi_device_handle_t dev_handle) {
  ...
  ESP_ERROR_CHECK(
      uwb_write_reg(DWM_REG_DELAYED_SEND_RECEIVE, (uint8_t *)&delayed_time,
                    DWM_SIZE_BYTES_DELAYED_SEND_RECEIVE, dev_handle));

  // Read it back to verify
  uint8_t readback[5];
  ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_DELAYED_SEND_RECEIVE, readback,
                               DWM_SIZE_BYTES_DELAYED_SEND_RECEIVE,
                               dev_handle));
  ...
  if (readback_time != delayed_time) {
    log("ERROR: Readback doesn't match! Register write failed or wrong "
        "register!");
  }
  ...
  log("Actual TX time:       %llu (0x%llX)", current_time_after,
      current_time_after);
  log("Elapsed time in loop: %llu DTU (%.3f ms)", elapsed_dtu,
...
esp_err_t uwb_receive(uint8_t *rx, size_t len, spi_device_handle_t dev_handle) {
  ...
  uint64_t sys_status_mask = (1 << 13) | (1 << 10) | (1 << 18) | (1 << 12) |
                             (1 << 15) | (1 << 14) | (1 << 16);
  ESP_ERROR_CHECK(uwb_write_reg(DWM_REG_SYSTEM_EVENT_STATUS,
                                (uint8_t *)&sys_status_mask, 5, dev_handle));
  ...
  ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_SYSTEM_CONTROL, (uint8_t *)&sys_ctrl,
                               sizeof(sys_ctrl), dev_handle));
  ...
  do {
    ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_SYSTEM_EVENT_STATUS,
                                 (uint8_t *)sys_status, sizeof(sys_status),
                                 dev_handle));
    log("Showing sys_status: %X %X %X %X %X", sys_status[4], sys_status[3],
        sys_status[2], sys_status[1], sys_status[0]);
    vTaskDelay(pdTICKS_TO_MS(1)); // prevent task starvation
  } while (((sys_status[1] >> 5) & 1) == 0);

  ESP_ERROR_CHECK(uwb_read_reg(DWM_REG_RECEIVE_DATA_BUFFER, (uint8_t *)rx, len,
                               dev_handle));
