...
Sensor::Sensor(uint32_t id, const char *name, uint32_t period_ms)
    : sensor_id(id) {
  if (Sensor::sensorTaskHandle != nullptr) {
    TimerHandle_t timer = xTimerCreate(name, pdMS_TO_TICKS(period_ms), pdTRUE,
                                       (void *)id, Sensor::registerSensorTimer);
